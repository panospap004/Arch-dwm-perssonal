#!/bin/bash
# WORKS
# Rofi Emoticons with Greenclip Integration
...
# Extract emoji data from the script
emoji_data=$(sed '1,/^# # DATA # #$/d' "$0")

# Use rofi to select an emoji
selected_emoji=$(echo "$emoji_data" | rofi -i -dmenu -config ~/.config/rofi/config-emoji.rasi | awk -F'\t' '{print $1}')

# If an emoji is selected, copy it to the clipboard
