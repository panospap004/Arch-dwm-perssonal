...
drawdate(int x, int y, int w)
{
    ...
    strftime(date, sizeof(date), "%H:%M :: %A %d / %B %m / %Y", tm);
...
drawmenu(void)
{
	...
	if (lines > 0) {
		...
		/* draw grid */
		int i = 0;
    int item_x_offset = (!draw_input && prompt && *prompt) ? 0 : x;
    int item_width = (!draw_input && prompt && *prompt) ? mw : (mw - x);
		for (item = curr; item != next; item = item->right, i++)
			drawitem(
				item,
        item_x_offset + ((i / lines) * (item_width / columns)),
      ...
      drawdate(item_x_offset, (lines + 1) * bh, item_width);
	} else if (matches) {
...
qalc_init(void)
{
	pipe(qalc.in);
  pipe(qalc.out);
  fcntl(qalc.out[0], F_SETFL, O_NONBLOCK);
  fcntl(qalc.out[1], F_SETFL, O_NONBLOCK);
	qalc.pid = fork();
	if (qalc.pid == -1)
...
usage(void)
{
	...
	die("usage: dmenu [-bCFfiv] [-noi] [-l lines] [-p prompt] [-fn font] [-m monitor]\n"
