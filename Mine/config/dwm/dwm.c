...
static void setdwmdesktops(char *tagState, Monitor *m);
...
static void attachside(Client *c);
...
{
    if (usealtbar) {
        // Generate tag state string for polybar
        ...
        for(i = 0; i < TAGSLENGTH; i++) {
            tagState[i] = 'e';  // empty
            if (occ & (1 << i)) {tagState[i] = 'o';}  // occupied
            if (urg & (1 << i)) {tagState[i] = 'u';}  // urgent
            if (m->tagset[m->seltags] & (1 << i)) {tagState[i] = 'a';}  // active
        }
        
        setdwmdesktops(tagState, m);
        free(tagState);
        return;
    }

    // Original bar drawing code here (if usealtbar is false)
    // ... rest of your original drawbar function
}
...
//     {
//       setsid();
//       execl("/bin/sh", "sh", "-c", altbarcmd, (char*)NULL);
//       exit(0);
//     }
//   }
// }
...
tagtoleft(const Arg *arg) {
	...
	&& selmon->tagset[selmon->seltags] > 1) {
		unsigned int newtags = selmon->sel->tags >> 1;
...
void
setdwmdesktops(char *tagState, Monitor *m)
{
    char atom[40];
    sprintf(atom, "DWM_TAG_STATE_%d", m->num);
    XChangeProperty(dpy, root, XInternAtom(dpy, atom, False), XA_STRING, 8, 
                   PropModeReplace, (unsigned char *)tagState, TAGSLENGTH);
}
